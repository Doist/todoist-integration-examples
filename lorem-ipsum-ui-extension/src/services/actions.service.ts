...
export class ActionsService {
    ...
    async process(request: DoistCardRequest, appToken: string | undefined): Promise<DoistCardResponse> {
        if (request.extensionType === 'context-menu' && request.action.actionType === 'initial') {
            return this.addTasksInputForm(request, appToken)
        
        } else if (request.extensionType === 'context-menu' && request.action.actionType === 'submit' 
    ...
    async addTasksInputForm(request: DoistCardRequest, appToken: string | undefined): Promise<DoistCardResponse> | never {
        if (!appToken) {
    ...
    async addTasks(request: DoistCardRequest, appToken: string | undefined): Promise<DoistCardResponse> | never {
        if (!appToken) {
            throw new Error('No AppToken') // TODO Do something better here
        }
        ...
        try {
            ...
            const addTaskCalls = [...Array(tasksNumber).keys()].map(() => todoistClient.addTask(
                {
                    content: loremIpsum(),
                    projectId: projectId,
                }
    ...
    async getContent(request: DoistCardRequest, appToken: string | undefined): Promise<DoistCardResponse> | never {
        if (!appToken) {
            throw new Error('No AppToken') // TODO Do something better here
        }
    ...
    async getSettings(request: DoistCardRequest, appToken: string | undefined): Promise<DoistCardResponse> | never {
        if (!appToken) {
            throw new Error('No AppToken') // TODO Do something better here
        }
